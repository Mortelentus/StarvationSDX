...
public class ItemActionSpawnMorteDog : ItemAction
{
    ...
    //{
    //    return (ItemActionData)new ItemActionSpawnMorteAnimal.ItemActionDataSpawnMinibike(_invData, _indexInEntityOfAction);
    //}
    ...
    public override void ReadFrom(DynamicProperties _props)
    {
        ...
        //Debug.Log("Adolesence: " + EntityAnimalChickenHal.Adolesence);
    ...
    public override void ExecuteAction(ItemActionData _actionData, bool _bReleased)
    {
        ...
        if (vector3.Equals((object)Vector3.zero))
            return;
        {
            SendData(vector3, entityToSpawn, _actionData.invData.itemValue.Clone(),
                _actionData.invData.holdingEntity.entityId, _actionData.invData.itemValue.Quality);
    ...
    private bool SendData(Vector3 vector3, string entitySpawn, ItemValue holdingItemValue, int playerID, int UID)
    {
        try
        {
            ...
            _es.pos = vector3;
            //_es.id = entityId;
            _es.entityName = entitySpawn;
            _es.entityClass = EntityClass.FromString(entitySpawn);
            _es.onGround = true;
            _es.rot = Vector3.zero;
            int uid = UID;
            uid = Convert.ToInt32(holdingItemValue.Meta.ToString("0000") + holdingItemValue.Quality.ToString("0000"));
            if (uid <= 0) uid = 100;
            _es.lifetime = (float)uid;
            _es.belongsPlayerId = uid;
            _es.teamNumber = uid;
            Debug.Log("Going to send the package with " + entitySpawn + " and UID = " + uid + " meta = " +
                      holdingItemValue.Meta + " quality = " + holdingItemValue.Quality + " numuses = " +
                      holdingItemValue.UseTimes);
